...
const ACCESS_LABELS = {
  ...
  asl_interpreter: "ASL / interpreter support",
};
...
export function rankSpecialists(parsed, patient, options = {}) {
  ...
  const langPref = (patient.language_preference || "").trim();
  // Any specific language other than English / no-preference is a real constraint.
  const wantSpecificLang = !!langPref && !/^(english|no preference)$/i.test(langPref);
  ...
  const candidates = SPECIALISTS
    ...
    .map((provider) => {
      ...
      // language — supports English/French and any free-typed "Other" language.
      const hasLang =
        wantSpecificLang &&
        provider.languages.some((l) => l.toLowerCase() === langPref.toLowerCase());
      let language_score = 1;
      if (wantSpecificLang) {
        language_score = hasLang ? 1 : 0.2;
        if (hasLang) explanation.push(`${langPref}-speaking provider`);
        else tradeoffs.push(`No ${langPref}-speaking service (patient prefers ${langPref})`);
      }
      ...
      const constraintMiss = (wantSpecificLang && !hasLang) || unmet.length > 0;
